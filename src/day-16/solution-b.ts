...
function calculateTunnelsDistance(firstHalf: Valve[], secondHalf: Valve[]) {
  let distance = 0;
  for (let i = 0; i < firstHalf.length; i++) {
    for (let j = i + 1; j < firstHalf.length; j++) {
      distance += distanceBetween(firstHalf[i], firstHalf[j]);
    }
  }
  for (let i = 0; i < secondHalf.length; i++) {
    for (let j = i + 1; j < secondHalf.length; j++) {
      distance += distanceBetween(secondHalf[i], secondHalf[j]);
    }
  }
...
function bestTunnelsSplit(valves: Valve[]) {
  ...
  for (let i = 0; i < 1_000_000; i++) {
    ...
    for (const valve of valves) {
      if (Math.random() > 0.5) {
        newFirstHalf.push(valve);
      } else {
        newSecondHalf.push(valve);
      }
    }
    const newDistance = calculateTunnelsDistance(newFirstHalf, newSecondHalf);
...
const [humanValves, elephantValves] = bestTunnelsSplit(highestTunnels);

let totalPressure = 0;

totalPressure += bestNextValve(valves.get("AA"), 26, humanValves).totalPressure;
totalPressure += bestNextValve(valves.get("AA"), 26, elephantValves).totalPressure;

console.log("Solution:", totalPressure);