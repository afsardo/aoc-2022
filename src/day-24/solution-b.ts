...
function mapKeyToPoint(key: string) : Point {
  const [x, y] = key.split(",").map(Number);
  return { x, y };
}

type Solution = {point: Point, firstGoalReached: number, returnToStartReached: number};

function mapSolutionToKey(solution: Solution) : string {
  return `${solution.point.x},${solution.point.y},${solution.firstGoalReached},${solution.returnToStartReached}`;
}

function mapKeyToSolution(key: string) : Solution {
  const [x, y, firstGoalReached, returnToStartReached] = key.split(",").map(Number);
  return { point: { x, y }, firstGoalReached, returnToStartReached };
}
...
function draw(blizzards: Map<string, Blizzard[]> = allBlizzards, round: number = 0, expeditionSolutions: Set<string> = new Set<string>()) {
  console.clear();
  for (let y = 0; y < matrix.length; y++) {
    ...
    for (let x = 0; x < matrix[y].length; x++) {
      ...
      if (blizzards.has(key)) {
        ...
        line += "#";
      } else if (Array.from(expeditionSolutions).some((key) => mapKeyToSolution(key).point.x === x && mapKeyToSolution(key).point.y === y)) {
        line += "E";
      } else {
        line += ".";
      }
  ...
  console.log("Round:", round + 1, "Expedition solutions:", expeditionSolutions.size);
}
...
function solve(rounds: number = Infinity) {
  ...
  let expeditionSolutions = new Set<string>();
  expeditionSolutions.add(mapSolutionToKey({ point: start, firstGoalReached: 0, returnToStartReached: 0 }));

  let currentRound = 0;
  while (currentRound < rounds) {
    currentBlizzards = moveBlizzards(currentBlizzards);

    const newExpeditionSolutions = new Set<string>();
    for (const key of expeditionSolutions) {
      const solution = mapKeyToSolution(key);
      if (solution.point.x === goal.x && solution.point.y === goal.y) {
        if (!solution.firstGoalReached) {
          solution.firstGoalReached = 1;
        }
        if (solution.firstGoalReached && solution.returnToStartReached) {
          return currentRound;
        }
      }
      if (solution.point.x === start.x && solution.point.y === start.y && solution.firstGoalReached) {
        if (!solution.returnToStartReached) {
          solution.returnToStartReached = 1;
        }
      }

      const possiblePoints = possibleMoves(solution.point, currentBlizzards);
      for (const possiblePoint of possiblePoints) {
        newExpeditionSolutions.add(mapSolutionToKey({
          point: possiblePoint,
          firstGoalReached: solution.firstGoalReached,
          returnToStartReached: solution.returnToStartReached,
        }));
      }
    }
    expeditionSolutions = newExpeditionSolutions;

    currentRound++;
    // draw(currentBlizzards, currentRound, expeditionSolutions);
    // sleepSync(1 / 240);
  }
