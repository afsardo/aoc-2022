...
for (const line of input.split("\n")) {
  ...
  if (
    ...
    (pairAStart <= pairBStart && pairAStart <= pairBEnd && pairAEnd >= pairBStart && pairAEnd <= pairBEnd) ||
    (pairBStart >= pairAStart && pairBEnd <= pairAEnd) ||
    (pairBStart <= pairAStart && pairBStart <= pairAEnd && pairBEnd >= pairAStart && pairBEnd <= pairAEnd)
  ) {
    count++;
  }
}
