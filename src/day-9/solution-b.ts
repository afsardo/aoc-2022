...
function moveTail(knotIndex: number = 1) {
  if (knotIndex === 0) throw new Error("knotIndex must be greater than 0");

  const head = knots[knotIndex - 1];
  const tail = knots[knotIndex];

  if (tail.isAdjacentOrOverlapping(head)) return;

  const moves = calculateMoves(head, tail);
  for (const move of moves) {
    for (let i = 0; i < move.distance; i++) {
      tail.move(move.direction)
    }
  }

  if (knotIndex + 1 === knots.length) {
    positionsTailVisted.add(new Position(tail.x, tail.y));
  }
...
console.log("Solution:", positionsTailVisted.size + 1);